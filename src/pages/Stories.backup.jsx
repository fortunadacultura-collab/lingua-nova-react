...
const Stories = () => {
  ...
  const loadStoryTxt = async (storyId, language = 'en') => {
    try {
      const response = await fetch(`/stories/${language}/${storyId}.txt`);
      if (!response.ok) {
  ...
  const playPhrase = (index) => {
    ...
    console.log(`▶️ Reproduzindo frase ${index}`);
    ...
    // Reproduzir a partir desta frase
    playCurrentPhrase();
    startProgressTracking();
  };
  ...
  const highlightCurrentPhrase = () => {
    ...
    const phraseElement = document.querySelector(`[data-index="${currentPhraseIndex}"]`);
    if (phraseElement) {
        ...
        <div className="story-container">
          ...
          <div id="story-content">
            {currentStoryData?.phrases?.map((phrase, index) => (
              <div 
                key={index} 
                className={`paragraph ${currentPhraseIndex === index && isPlaying ? 'highlighted' : ''}`}
                data-index={index}
                onClick={(e) => {
